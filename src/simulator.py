...
class StocksSimulator(gym.Env):
    ...
    def handle_orders(self):
        ...
        for company, order in self.orders.items():
            ...
            if complete:
                ...
                if self.log_transactions:
                    common.log("capital:", self.get_capital(), ", reward:", self.total_reward)
            else:
    ...
    def step(self, action):
        ...
        ch = (self.capital / prev_capital - 1) + (self.capital / self.cash_init - 1) / 1000000
        ch = ch if ch <= 0 else 1 - 1 / (ch + 1)
        reward = math.copysign(1, ch) * math.sqrt(abs(ch)) * 100000 #- .00006
